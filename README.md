...
- **On your status line**, as a live `5h:11%(3h:41m) 7d:3%(5.2d)` readout —
...
```
~ ctx:10% Opus 4.8 (1M context) 5h:11%(3h:41m) 7d:3%(5.2d)
                                 └────────────────────────┘ ccgauge
```
...
- **Onto the status line.** `usage.py status` prints a short coloured
  `5h:X%(Xh:Ym) 7d:Y%(X.Yd)` fragment — utilization colour-coded (green < 70,
